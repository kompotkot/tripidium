...
func (p *PsqlDB) GetUser(ctx context.Context, userId, email string) (iam.User, error) {
	var user iam.User
	var err error
...
func (p *PsqlDB) GetToken(ctx context.Context, tokenId string) (iam.Token, error) {
	query := `SELECT id, user_id, active, created_at, updated_at FROM tokens WHERE id = $1`
