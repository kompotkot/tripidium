...
func (h *Handler) assertNotLastOwner(ctx context.Context, orgID, callerID uuid.UUID) error {
	members, err := h.deps.DB.ListOrganizationMembers(ctx, orgID, callerID)
	if err != nil {
		return err
	}
	ownerCount := 0
...
func (h *Handler) ListOrganizations(w http.ResponseWriter, r *http.Request) {
	...
	orgs, err := h.deps.DB.ListOrganizations(r.Context(), cID)
	if err != nil {
...
func (h *Handler) CreateOrganization(w http.ResponseWriter, r *http.Request) {
	...
	orgID := uuid.New()
	org, err := h.deps.DB.CreateOrganization(r.Context(), orgID, name, description, cID)
	if err != nil {
...
func (h *Handler) GetOrganization(w http.ResponseWriter, r *http.Request) {
	...
	org, err := h.deps.DB.GetOrganization(r.Context(), orgID, cID)
	if err != nil {
	...
	members, err := h.deps.DB.ListOrganizationMembers(r.Context(), orgID, cID)
	if err != nil {
	...
	out.Members = make([]dto.OrganizationMemberResponse, 0, len(members))
	for _, m := range members {
...
func (h *Handler) UpdateOrganization(w http.ResponseWriter, r *http.Request) {
	...
	org, err := h.deps.DB.UpdateOrganization(r.Context(), orgID, cID, req.Name, req.Description)
	if err != nil {
		if errors.Is(err, db.ErrOrganizationNotFound) {
			http.Error(w, "Organization not found", http.StatusNotFound)
			return
...
func (h *Handler) DeleteOrganization(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.deps.DB.DeleteOrganization(r.Context(), orgID, cID); err != nil {
		if errors.Is(err, db.ErrOrganizationNotFound) {
			http.Error(w, "Organization not found", http.StatusNotFound)
			return
		}
...
func (h *Handler) ListMembers(w http.ResponseWriter, r *http.Request) {
	...
	out := dto.OrganizationMemberListResponse{
		ID:      orgID.String(),
		Members: make([]dto.OrganizationMemberResponse, 0, len(members))}
	for _, m := range members {
		out.Members = append(out.Members, ToOrganizationMemberResponse(m))
	}
	w.Header().Set("Content-Type", "application/json")
...
func (h *Handler) AddMember(w http.ResponseWriter, r *http.Request) {
	...
	callerMember, err := h.deps.DB.GetOrganizationMember(r.Context(), orgID, cID, cID)
	if err != nil {
	...
	member, err := h.deps.DB.AddOrganizationMember(r.Context(), orgID, targetUserID, cID, req.Role)
	if err != nil {
...
func (h *Handler) GetMember(w http.ResponseWriter, r *http.Request) {
	...
	member, err := h.deps.DB.GetOrganizationMember(r.Context(), orgID, targetUserID, cID)
	if err != nil {
...
func (h *Handler) UpdateMemberRole(w http.ResponseWriter, r *http.Request) {
	...
	targetMember, err := h.deps.DB.GetOrganizationMember(r.Context(), orgID, targetUserID, cID)
	if err != nil {
		if errors.Is(err, db.ErrOrganizationMemberNotFound) {
			http.Error(w, "Member not found", http.StatusNotFound)
			return
	...
	member, err := h.deps.DB.UpdateOrganizationMemberRole(r.Context(), orgID, targetUserID, cID, req.Role)
	if err != nil {
		if errors.Is(err, db.ErrOrganizationMemberNotFound) {
			http.Error(w, "Member not found", http.StatusNotFound)
			return
...
func (h *Handler) RemoveMember(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.deps.DB.RemoveOrganizationMember(r.Context(), orgID, targetUserID, cID); err != nil {
		if errors.Is(err, db.ErrOrganizationMemberNotFound) {
			http.Error(w, "Member not found", http.StatusNotFound)
			return
		}
